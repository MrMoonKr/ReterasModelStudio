...
/**
 * 메인프레임 {@link MainFrame}. Write a description of class MainFrame here.
 *
 * @author (your name)
...
public class MainFrame extends JFrame {
    ...
    private static final String RETERA_MODEL_STUDIO_VERSION = "Retera Model Studio v0.04.5m";
    ...
    public static void main( final String[] args ) {
        ...
        if ( hasArgs ) {
            if ( args.length > 1 && args[0].equals( "-convert" ) ) {
                ...
                final boolean alwaysUseMinimalMatricesInHD = SaveProfile.get().getPreferences()
                        .isAlwaysUseMinimalMatricesInHD();
                if ( mdxInput || mdlInput || objInput ) {
                    ...
                    try {
                        ...
                        if ( destination.toLowerCase().endsWith( ".tga" ) ) {
                            ...
                            ImageIO.write( sourceImage, destination.substring( destination.lastIndexOf( '.' ) + 1 ),
                                    result );
                        }
                        System.out.println( RETERA_MODEL_STUDIO_VERSION + " converted: " + result );
                    } catch ( final IOException e ) {
                    ...
                    System.out.println( "Unable to convert."
                            + " If you are converting an image, provide an output path to tell me what file extension to save as." );
                }
                return;
        ...
        try {
            ...
            case JAVA_DEFAULT:
                ...
                // UIManager.getLookAndFeel().getDefaults().put("TabbedPane.background",
                // Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("InternalFrame.activeTitleBackground",
                // Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("InternalFrame.activeTitleForeground",
                // Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("InternalFrame.inactiveTitleBackground",
                // Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("InternalFrame.inactiveTitleForeground",
                // Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("Button.select", Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("Button.disabledText",
                // Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("ScrollBar.background",
                // Color.GREEN);
                // UIManager.getLookAndFeel().getDefaults().put("ScrollBar.shadow",
                // Color.GREEN);
                break;
            case SOFT_GRAY:
                try {
    ...
    public MainFrame( final String title ) {
        ...
        // currentThread.setDefaultUncaughtExceptionHandler( new
        // UncaughtExceptionHandler() {
        // @Override
        // public void uncaughtException( final Thread t, final Throwable e ) {
        // ExceptionPopup.display( "Uncaught exception on Thread \"" + t.getName() +
        // "\"", e );
