...
public class NGGLDP {
    ...
    /**
     * 디퓨즈 셰이더 기반 렌더링 파이프라인. OpenGL 3.3
    ...
    public static final class SimpleDiffuseShaderPipeline implements Pipeline {
        ...
        private FloatBuffer pipelineVertexBuffer = ByteBuffer.allocateDirect( 1024 * 4 )
        ...
        private void ensureCapacity( final int absoluteOffset ) {
            if ( pipelineVertexBuffer.capacity() <= absoluteOffset ) {
                final FloatBuffer largerBuffer = ByteBuffer
                        .allocateDirect(
                                Math.max( ( absoluteOffset + 1 ) * 4, pipelineVertexBuffer.capacity() * 2 * 4 ) )
    ...
    /**
     * PBR 셰이더 기반 렌더링 파이프라인. OpenGL 3.3
    ...
    public static final class HDDiffuseShaderPipeline implements Pipeline {
        ...
        private static final int STRIDE = 4 /* position */ + 4 /* normal */ + 4 /* tangent */ + 2 /* uv */ + 4 /* color */ ;
        private static final int STRIDE_BYTES = STRIDE * Float.BYTES;
        private static final String vertexShader = "#version 330 core\r\n" + //
                "\r\n" + //
                "layout (location = 0) in vec4 a_position;\r\n" + //
        ...
        /**
         * 셰이더 객체 생성.
         * @param shaderType 셰이더 타입 ( GL20.GL_VERTEX_SHADER, GL20.GL_FRAGMENT_SHADER )
         * @param shaderSource 셰이더 소스코드
         * @return
         */
        private int createShader( final int shaderType, final String shaderSource ) {
            final int shaderId = GL20.glCreateShader( shaderType );
        ...
        private void ensureCapacity( final int absoluteOffset ) {
            if ( pipelineVertexBuffer.capacity() <= absoluteOffset ) {
                final int newSizeBytes = Math.max( ( absoluteOffset + 1 ) * 4,
                        pipelineVertexBuffer.capacity() * 2 * 4 );
                final FloatBuffer largerBuffer = ByteBuffer.allocateDirect( newSizeBytes )
                        .order( ByteOrder.nativeOrder() ).asFloatBuffer().clear();
                pipelineVertexBuffer.flip();
                largerBuffer.put( pipelineVertexBuffer );
                largerBuffer.clear();
        ...
        public void glEnd() {
            ...
            // GL20.glUniform3f(GL20.glGetUniformLocation(shaderProgram,
            // "u_lightDirection"), 0.0f, 0.0f, -10000f);

            GL20.glUniform3f( GL20.glGetUniformLocation( shaderProgram, "u_viewPos" ), cameraLocation.x,
                    cameraLocation.y, cameraLocation.z );
            GL20.glUniform2f( GL20.glGetUniformLocation( shaderProgram, "u_viewportSize" ), viewportWidth,
                    viewportHeight );
            GL20.glUniform1f( GL20.glGetUniformLocation( shaderProgram, "u_fresnelTeamColor" ), fresnelTeamColor );
            GL20.glUniform4f( GL20.glGetUniformLocation( shaderProgram, "u_fresnelColor" ), fresnelColor.x,
                    fresnelColor.y, fresnelColor.z, fresnelOpacity );
