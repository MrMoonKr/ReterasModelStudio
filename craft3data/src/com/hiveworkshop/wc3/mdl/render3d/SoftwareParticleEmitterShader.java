...
public class SoftwareParticleEmitterShader implements ParticleEmitterShader {
    ...
    @Override
    public void renderParticles( final int blendSrc, final int blendDst, 
            final int rows, final int cols,
            final InternalResource texture, 
            final float[] bufferData, final boolean isRibbonEmitter,
            final int numItemsToRender ) 
    {
        ...
        if ( ( numItemsToRender * 5 ) > bufferData.length ) {
            // throw new IllegalStateException(numItemsToRender + " alive items, " +
            // bufferData.length + " len buffer");
        ...
        for ( int itemIndex = 0; itemIndex < numItemsToRender; itemIndex++ ) {
            ...
            // System.out.println((byte) ((colorInt >> 16) & 0xFF) + "," + (byte) ((colorInt
            // >> 8) & 0xFF) + ","
            // + (byte) ((colorInt) & 0xFF) + "," + (byte) ((uvInt) & 0xFF));
