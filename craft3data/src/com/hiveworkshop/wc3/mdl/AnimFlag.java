...
public class AnimFlag {
    ...
    public AnimFlag( final MaterialFresnelColor cornColor ) {
        ...
        for ( final MaterialFresnelColor.ScalingTrack track : cornColor.scalingTrack ) {
            if ( tans ) {
                addEntry( track.time, new Vertex( track.color ), new Vertex( track.inTan ),
    ...
    public AnimFlag( final TextureScaling textureData ) {
        ...
        for ( final TextureScaling.TranslationTrack track : textureData.translationTrack ) {
            if ( tans ) {
                addEntry( track.time, new Vertex( track.scaling ), new Vertex( track.inTan ),
                        new Vertex( track.outTan ) );
            } else {
    ...
    public AnimFlag( final TextureTranslation textureData ) {
        ...
        for ( final TextureTranslation.TranslationTrack track : textureData.translationTrack ) {
            if ( tans ) {
                addEntry( track.time, new Vertex( track.translation ), new Vertex( track.inTan ),
                        new Vertex( track.outTan ) );
            } else {
    ...
    public AnimFlag( final LightAmbientColor trackData ) {
        ...
        for ( final LightAmbientColor.ScalingTrack track : trackData.scalingTrack ) {
            if ( tans ) {
                addEntry( track.time, new Vertex( track.ambientColor ), new Vertex( track.inTan ),
                        new Vertex( track.outTan ) );
            } else {
    ...
    public void updateGlobalSeqRef( final EditableModel mdlr ) {
        if ( hasGlobalSeq ) {
            if ( globalSeqId >= 0 && globalSeqId < mdlr.getGlobalSeqs().size() ) {
    ...
    private int ceilIndex( final int time, final int lo, final int hi ) {
        ...
        } else if ( mid - 1 >= lo && time > times.get( mid - 1 ) ) {
            return mid;
        } else {
    ...
    public Object interpolateAt( final AnimatedRenderEnvironment animatedRenderEnvironment ) {
        return interpolateAt( animatedRenderEnvironment, LayerShader.SD );
    }

    public Object interpolateAt( final AnimatedRenderEnvironment animatedRenderEnvironment, final Object identity ) {
        return interpolateAt( animatedRenderEnvironment, identity, LayerShader.SD );
    }
     ...
     */
    public Object interpolateAt( final AnimatedRenderEnvironment animatedRenderEnvironment,
            final LayerShader shaderType ) {
        return interpolateAt( animatedRenderEnvironment, getIdentity() );
    }
    ...
    public Object interpolateAt( final AnimatedRenderEnvironment animatedRenderEnvironment, final Object identity,
        ...
        if ( hasGlobalSeq() && getGlobalSeq() >= 0 ) {
            ...
            if ( floorAnimStartIndex == floorAnimEndIndex && floorAnimStartIndex < times.size()
                    && times.get( floorAnimStartIndex ) >= animation.getStart()
                    && times.get( floorAnimStartIndex ) <= animation.getEnd() ) {
        ...
        case ALPHA | OTHER_TYPE: {
            ...
            InterpolationType interpTypeAsEnum = getInterpTypeAsEnum();
            if ( shaderType == LayerShader.HD ) {
                if ( localTypeId == ALPHA && interpTypeAsEnum == InterpolationType.DONT_INTERP ) {
                    interpTypeAsEnum = InterpolationType.LINEAR;
                }
            }
            switch ( interpTypeAsEnum ) {
            case BEZIER: {
        ...
        case COLOR: {
            ...
            case LINEAR:
                ...
                final Vertex lerp = new Vertex( MathUtils.lerp( previous.x, next.x, timeFactor ),
                        MathUtils.lerp( previous.y, next.y, timeFactor ),
                        MathUtils.lerp( previous.z, next.z, timeFactor ) );
                return lerp;
            default:
                throw new IllegalStateException();
            }
        ...
        case ROTATION: {
            ...
            case BEZIER: {
                ...
                return QuaternionRotation.ghostwolfSquad( result, previous, previousOutTan, nextInTan, next,
                        timeFactor );
            }
            case DONT_INTERP:
                return floorValue;
